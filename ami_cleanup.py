...
def lambda_handler(event, context):
    '''Clean AMIs and its associated SnapShots which are older than 7 Days and without "Exclude = No" tag in a AWS Region
    ...
    for i in ami_response['Images']:
        for tag in i.get('Tags', ''):
            if tag['Key'] == 'Exclude' and tag['Value'] == 'No' in tag.values():
